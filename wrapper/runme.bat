...
kubectl exec -n artifactory %ART_POD_NAME% -- curl -uadmin:password -XPOST -H "Content-Type: application/json" -d "{ \"licenseKey\": \"\" }" http://localhost:8081/artifactory/api/system/licenses
kubectl exec -n artifactory %ART_POD_NAME% -- curl -uadmin:password -XPUT -H "Content-Type: application/json" -d "{ \"email\": \"e@mail.com\", \"password\": \"Aa123456\", \"admin\": true }" http://localhost:8082/artifactory/api/security/users/tomer
kubectl exec -n artifactory %ART_POD_NAME% -- curl -uadmin:password -XPUT -H "Content-Type: application/json" -d "{ \"key\": \"miniserver\", \"rclass\" : \"local\", \"packageType\": \"docker\" }" http://localhost:8081/artifactory/api/repositories/miniserver
kubectl exec -n artifactory %ART_POD_NAME% -- curl -uadmin:password -XPOST http://localhost:8082/artifactory/api/plugins/reload
kubectl exec -n artifactory %ART_POD_NAME% -- curl -uadmin:password -XPOST http://localhost:8082/artifactory/api/plugins/execute/webhookReload
